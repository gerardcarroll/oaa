...
package com.overstock.android.prototype.espresso.testrules;

import static com.overstock.android.prototype.espresso.common.PerfTestingUtils.getTestFile;
...
/**
 * This rule resets battery stats before a test and executes a dumpsys for batterystats after performing the test. If
 * the API level is less than 21 then this rule will do nothing since this dumpsys command isn't supported. It has
 * limited use for short tests and is meant for tests you would typically mark as Large. For short tests you can
 * manually use this in a {@code org.junit.runner.notification.RunListener}.
 *
 * <pre>
 * &#64;Rule
    ...
    if (android.os.Build.VERSION.SDK_INT >= 21) {
      ...
      try {
        ...
        if (mLogFileAbsoluteLocation == null) {
          mLogFileAbsoluteLocation = getTestFile(mTestClass, mTestName, "battery.dumpsys.log");
        }
        ...
        if (process.exitValue() != 0) {
          throw new Exception("Error while taking dumpsys, exitCode=" + process.exitValue());
        }
      }
      catch (Exception exception) {
