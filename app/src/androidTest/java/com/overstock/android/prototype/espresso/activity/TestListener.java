/*
 * Copyright 2015, Google Inc. Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file
...
import com.overstock.android.prototype.espresso.common.PerfTestingUtils;
import com.overstock.android.prototype.espresso.testrules.EnableBatteryStatsDump;
import com.overstock.android.prototype.espresso.testrules.EnableDeviceGetPropsInfo;

/**
 * Initiate pre-test and post-test procedures; including cleaning and moving files from the internal app data directory
 * to the external one. Also perform writing to test-start and test-end files which are used to indicate whether there
 * was a fatal test exception (e.g. OutOfMemoryException).
 */
public class TestListener extends RunListener {
  ...
  // The following two methods enable log files to be pulled as well as battery and location request information
  // to be requested.
  @Override
  public void testRunStarted(Description description) throws Exception {
    ...
    mEnableBatteryStatsDump = new EnableBatteryStatsDump(PerfTestingUtils.getTestRunFile("batterystats.dumpsys.log"));
    mEnableBatteryStatsDump.before();

    mEnableDeviceGetPropsInfo = new EnableDeviceGetPropsInfo(PerfTestingUtils.getTestRunFile("getprops.log"));
  ...
  /**
   * Move files from the app's internal file location to a location that can be read on retail devices with simple ADB
   * pull commands.
   */
  private void copyTestFilesToExternalData() throws IOException, InterruptedException {
    ...
    if (process.exitValue() != 0) {
      ...
      throw new IOException("Not able to move test data to external storage directory:" + " src=" + srcAbsolutePath
  ...
  private void dumpLocationRequestInformation() {
    try {
      ...
      File externalTestFilesStorageDir = getAppDataLogDir();
      File locationRequestLogFile = new File(externalTestFilesStorageDir, "locationRequests" + ".dumpsys.log");
  ...
  private void logTestFailure(Failure failure) throws IOException, InterruptedException {
    File failureLogFile = PerfTestingUtils.getTestFile(failure.getDescription().getClassName(),
