...
public class EnableLogcatDump extends ExternalResource {
  ...
  public void after() {
    try {
      ...
      processBuilder.command("logcat", "-d", "-f", getTestFile(mTestClass, mTestName, "logcat.log").getAbsolutePath());
      processBuilder.redirectErrorStream();
      Process process = processBuilder.start();
      process.waitFor();
      if (process.exitValue() != 0) {
        Log.e(LOG_TAG, "Error exit value while extracting logcat, exitValue=" + process.exitValue());
      }
    }
